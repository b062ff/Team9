...
if(isset($_POST["SelectHome"])){
    ...
    if(!count($home_list))$error_message[]="メッセージなし";
        ...
        $json_array = json_encode($home_list);
    }
                    ...
                    <div class="sample03">
                        <ul>
                            <?php foreach($home_list as $home){ ?>
                                ...
                                <div class="messa"><?php echo($home["body"]);?></div>
                                <form id="voice-form"> 
                                    <?php  $speech=$home["body"]; $Pitch = $home["pich"];?>
                                    <div><input type="button" class="circlebutton"  value="再生"  id="submitBtn" v-on:click="submit" :disabled="!isWork" onclick="ToSayClick(<?php echo($VT); ?>,<?php echo($Pitch); ?>,'<?php echo($speech); ?>');"/></div>
                                </form>
                                </div>
                            <?php } ?>
                        </ul>
